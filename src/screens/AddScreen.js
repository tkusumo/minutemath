...
class AddScreen extends Component {
  ...
  _generateAnswers = (firstNumber, secondNumber) => {
    ...
    if (this.state.level === 'Easy') {
      ...
      min = (Math.ceil((firstNumber + secondNumber) / 10) * 10) - 20;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      while (answers.find(function (element) {
          return element === rand;
        }
      ))
        {
  ...
  _generateProblemAndSolution = () => {
    ...
    const firstNumber = this._generateRandomInt(firstMin, firstMax);
    const secondNumber = this._generateRandomInt(secondMin, secondMax);
      ...
      <View style={styles.container}>
        ...
        <View style={{ paddingBottom: 30 }}>
          <Text style={[styles.title, { color: this.state.themeColor.text }]}>
            {this.state.questionCount === 0 ? 'Score' : 'Addition'}
          </Text>
        </View>
        <TouchableWithoutFeedback
